  ...
  List<DataSourceConfig> getDataSourceConfigs() {
    return [
      ...
      DataSourceConfig(
        ...
        source: 'Google Drive',
        format: 'Shapefile/ZIP',
        description: 'Migratory fish spawning and nursery zones (Custom dataset)',
        isAvailable: true,
        localPath: path.join(_dataDirectory?.path ?? '', 'shapefiles', 'fish_habitat'),
        metadata: {'total_zones': 2, 'file_id': '1d2Qhco6c8VfFju38iVJE9lLhyX4hzQKV'},
      ),
      DataSourceConfig(
        name: 'SAV Zones',
        type: 'shapefile',
        source: 'Google Drive',
        format: 'Shapefile/ZIP',
        description: 'Submerged aquatic vegetation zones (Custom dataset)',
        isAvailable: true,
        localPath: path.join(_dataDirectory?.path ?? '', 'shapefiles', 'sav_zones'),
        metadata: {'total_zones': 2, 'file_id': '19LDFpPB3XWVIg2LPvBmLdoZbjLbHjJ2Q'},
      ),
      DataSourceConfig(
        ...
        source: 'Google Drive',
        format: 'ZIP/Custom',
        description: 'Oil spill training dataset (Custom dataset)',
        isAvailable: true,
        localPath: path.join(_dataDirectory?.path ?? '', 'datasets', 'zenodo_oil_spill'),
        metadata: {'satellite': 'Sentinel-1', 'file_id': '10Tc2W8hvU6s3M5g_vjyzcfuVx4CoBZ_1'},
      ),
      DataSourceConfig(
        ...
        source: 'Google Drive',
        format: 'ZIP/Custom',
        description: 'Oil spill detection dataset (Custom dataset)',
        isAvailable: true,
        localPath: path.join(_dataDirectory?.path ?? '', 'datasets', 'm4d_oil_spill'),
        metadata: {'satellite': 'Sentinel-1', 'file_id': '1RXnJrfDr22nsjvmwfKb4OpTAL8E3wrFE'},
      ),
    ];
  }
